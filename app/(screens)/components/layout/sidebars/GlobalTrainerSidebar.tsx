...
import { House, Globe, PresentationChart, Barbell } from "@phosphor-icons/react";
import LogoutButton from "../../reusable/LogoutButton";

const navLinks = [
  { name: "Dashboard", href: "/globaltrainer", icon: House },
  { name: "Global Clients", href: "/globaltrainer/clients", icon: Globe },
  { name: "Programs", href: "/globaltrainer/programs", icon: PresentationChart },
];

export default function GlobalTrainerSidebar() {
    ...
    <div className="w-[256px] h-full bg-[#0E0F13] border-r-[2px] border-[#232631] flex flex-col justify-between items-start px-4 py-6">
      <div className="flex flex-col items-start gap-8 w-full">
        ...
        <div className="flex flex-col items-start gap-2 w-full">
          {navLinks.map((link) => {
            const isActive = pathname === link.href || (link.href !== "/globaltrainer" && pathname.startsWith(`${link.href}/`));
