  ...
  return (
    <div className="flex flex-col items-start p-4 md:p-5 gap-[15px] w-full bg-[#161B22] border border-[#14161A] shadow-[0_1px_2px_rgba(0,0,0,0.05)] rounded-xl">
      ...
      <div className="flex flex-col items-start gap-1.5 w-full">
        ...
        <div className={`flex flex-row justify-center items-start px-3 py-2 w-full min-h-[96px] bg-[#1A2029] border rounded-lg ${errors.bio ? "border-red-500" : "border-[#14161A]"}`}>
          <textarea
            {...register("bio")}
        ...
        {errors.bio && <span className="text-red-500 text-[10px]">{errors.bio.message as string}</span>}
      </div>
    </div>
  );
}
