...
export default function MonthlyRevenueChart({ data = [] }: MonthlyRevenueChartProps) {
  ...
  let normalizedStep = magnitude > 0 ? roughStep / magnitude : 1;
  if (isNaN(normalizedStep) || !isFinite(normalizedStep)) normalizedStep = 1;
  ...
  if (isNaN(niceStep) || !isFinite(niceStep) || niceStep === 0) niceStep = 1000;
          ...
          <div className="relative flex flex-row items-start min-w-[700px] h-[248px] pt-10 pb-8">
            <div className="relative flex-1 h-[176px] isolate">
              ...
              <div className="relative flex flex-row justify-around items-end px-2 h-full w-full z-10">
                {data.length > 0 ? (
                  data.map((item, index) => {
                    const heightPercent = yMax > 0 ? (item.value / yMax) * 100 : 0;
                    return (
                      <div
                        key={index}
