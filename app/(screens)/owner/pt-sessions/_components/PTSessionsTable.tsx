...
import { CaretRight, MagnifyingGlass, Eye } from "@phosphor-icons/react/dist/ssr";
...
export default function PTSessionsTable({
  ...
  const getStatusStyles = (status: SessionFilter | "-") => {
    switch (status) {
      ...
      case "-":
        return "bg-transparent border-transparent text-[#94A3B8]";
      default:
        ...
        <div className="flex flex-row justify-between items-center px-6 py-4 border-b border-[#19202C]">
          <div className="flex flex-row items-center gap-2">
            <span className="font-sans font-bold text-sm text-white">
              {new Date().toDateString() === selectedDate.toDateString() ? "Today" : selectedDate.toLocaleDateString('en-GB', { weekday: 'short' })}
            </span>
            <span className="font-sans font-normal text-xs text-[#4B5563]">•</span>
            <span className="font-sans font-medium text-xs text-[#7F8EA3]">
              {selectedDate.toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}
            </span>
          </div>
          <span className="font-sans font-semibold text-xs text-[#8B99AC]">{sessions.length} Sessions</span>
        </div>
                ...
                <tr key={session.id} className={`${index > 0 ? 'border-t border-[#171D27]' : ''} hover:bg-[#151A25] transition-colors`}>
                  ...
                  <td className="px-6 py-3.5 text-right">
                    <button
                      ...
                      className="inline-flex justify-center items-center w-8 h-8 rounded-[8px] hover:bg-[#1A202C] transition-colors cursor-pointer"
                    >
                      <Eye size={22} weight="regular" className="text-[#94A3B8]" />
                    </button>
                  </td>
                </tr>
              ))}
